...
export default function ApproveIntern() {
  const { token } = useParams();
  const navigate = useNavigate();
  const [status, setStatus] = useState('pending');
  const [message, setMessage] = useState('Approving intern...');
      ...
      <div className={`p-8 rounded-lg shadow-lg max-w-md w-full mx-4 ${
        ...
        <div className="text-center">
          ...
          <h2 className={`text-xl font-semibold mb-4 ${
            ...
            {status === 'success' ? 'Intern Approved!' : 
          ...
          {status !== 'pending' && (
            <div className="mt-4">
              ...
              <button
                onClick={() => status === 'success' ? navigate('/login') : navigate('/')}
                className="text-blue-600 hover:text-blue-800 text-sm underline"
              >
                {status === 'success' ? 'Go to Login Now' : 'Go to Home Page Now'}
              </button>
            </div>
          )}
        </div>
